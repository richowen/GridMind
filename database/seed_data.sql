...
-- Uses subquery to look up device by name rather than hardcoding immersion_id = 1,
-- so rules attach to the correct device regardless of insertion order.
...
SELECT
    ...
    'Negative Price', 1, 'ON', 'AND', TRUE, '<', 0.0
WHERE NOT EXISTS (
    SELECT 1 FROM immersion_smart_rules
    WHERE immersion_id = (SELECT id FROM immersion_devices WHERE name = 'main')
    AND rule_name = 'Negative Price'
);
...
SELECT
    (SELECT id FROM immersion_devices WHERE name = 'main'),
    'Very Cheap + Full Battery', 2, 'ON', 'AND', TRUE, '<', 2.0, TRUE, '>=', 95.0
WHERE NOT EXISTS (
    SELECT 1 FROM immersion_smart_rules
    WHERE immersion_id = (SELECT id FROM immersion_devices WHERE name = 'main')
    AND rule_name = 'Very Cheap + Full Battery'
);
...
SELECT
    (SELECT id FROM immersion_devices WHERE name = 'main'),
    'Solar Surplus', 3, 'ON', 'AND', TRUE, '>=', 5.0, TRUE, '>=', 90.0
WHERE NOT EXISTS (
    SELECT 1 FROM immersion_smart_rules
    WHERE immersion_id = (SELECT id FROM immersion_devices WHERE name = 'main')
    AND rule_name = 'Solar Surplus'
);
...
SELECT
    (SELECT id FROM immersion_devices WHERE name = 'main'),
    'Overheat Protection', 99, 'OFF', 'AND', TRUE, '>=', 70.0
WHERE NOT EXISTS (
    SELECT 1 FROM immersion_smart_rules
    WHERE immersion_id = (SELECT id FROM immersion_devices WHERE name = 'main')
    AND rule_name = 'Overheat Protection'
);
